...
type (
	...
	Characters struct {
		...
		VoiceId     int64  `db:"voice_id"`    // 音色id
	}
)
...
func (m *defaultCharactersModel) Insert(ctx context.Context, data *Characters) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Name, data.Subtitle, data.Description, data.Tags, data.Language, data.Greeting, data.Prompt, data.VoiceId)
	return ret, err
}
...
func (m *defaultCharactersModel) Update(ctx context.Context, data *Characters) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.Name, data.Subtitle, data.Description, data.Tags, data.Language, data.Greeting, data.Prompt, data.VoiceId, data.Id)
	return err
}
